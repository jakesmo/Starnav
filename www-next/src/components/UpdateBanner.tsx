...
export default function UpdateBanner({
  ...
  const dismiss = () => {
    setDismissed(true);
    try {
      sessionStorage.setItem("update-dismissed", latest);
    } catch {}
  };
      ...
      <div className="max-w-[1400px] mx-auto flex items-center justify-between gap-3">
        <div className="flex items-center gap-2 text-sm">
          ...
          <span className="text-text-primary">
            Update available on <code className="text-xs">{branch}</code>:{" "}
            <code className="text-xs">{current}</code> &rarr;{" "}
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            title="Open update modal to select branch and start update"
          >
            Update Now
          </button>
          <button
