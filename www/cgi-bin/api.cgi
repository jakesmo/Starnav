...
UPDATE_REPO_DIR="/opt/starnav"
. "$UPDATE_REPO_DIR/shared/update/backend/update-api.sh"

# Starnav-specific post-update hook (used by shared update-lib.sh)
update_post_apply() {
    local repo_dir="$1"

    # Fix permissions
    chmod +x "$repo_dir/starnav.sh" "$repo_dir/install.sh" 2>/dev/null
    chmod +x "$repo_dir/www/cgi-bin/"*.cgi 2>/dev/null

    # Restart service
    echo "  Restarting service..."
    if [ -x /etc/init.d/starnav ]; then
...
# Compat alias: Starnav case routing uses setup_log_action
setup_log_action() {
    get_update_log
}
